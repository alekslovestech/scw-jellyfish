...
async function postForm(url, formOrData) {
...
async function saveDeviceForm(url, formOrData) {
  const data = await postForm(url, formOrData);
  if (data.ok) {
...
function patternOptions(current) {
  return PATTERNS.map(([value, label]) =>
    `<option value="${value}"${value === current ? ' selected' : ''}>${label}</option>`
  ).join('');
}

function polarToXZ(radius, azimuthDeg) {
  const rad = azimuthDeg * Math.PI / 180;
  return { x: radius * Math.sin(rad), z: radius * Math.cos(rad) };
}

function xzToPolar(x, z) {
...
function render() {
  ...
  document.getElementById('fleetState').textContent = online
    ? `${online} controller${online === 1 ? '' : 's'} online`
    : 'No controllers online';

  const installation = state.installation || {};
  const expected = state.settings?.expectedPlatformCount || 0;
  ...
  document.getElementById('sceneState').textContent = installation.allCalm
    ? 'CHORUS'
    : String(state.settings?.mode || 'forest').toUpperCase();
  ...
  if (!state.settingsLoaded && state.settings) {
    setForm(document.getElementById('showForm'), state.settings);
  ...
  if (state.selectedKey && deviceDialog.open && !state.dialogDirty) {
    renderDialog(state.devices.find(d => d.key === state.selectedKey));
  }
}
...
function renderMap() {
  ...
  const minX = Number(s.mapMinX ?? -10), maxX = Number(s.mapMaxX ?? 10);
  const minZ = Number(s.mapMinZ ?? -10), maxZ = Number(s.mapMaxZ ?? 10);
  ...
  ctx.save();
  ctx.strokeStyle = '#4d99b1';
  ctx.fillStyle = '#a8cbdc';
  ctx.lineWidth = 2;
  ...
  for (const p of state.platforms) {
    ...
    const radius = (state.settings?.influenceRadiusMeters || 5) / (maxX - minX) * w;
    const halo = ctx.createRadialGradient(x, y, 4, x, y, Math.max(10, radius));
    halo.addColorStop(0, p.occupied ? (p.agitation > .2 ? '#ff718477' : '#ffe39a77') : '#ffe39a22');
    halo.addColorStop(1, 'transparent');
    ctx.fillStyle = halo;
    ctx.beginPath();
    ctx.arc(x, y, Math.max(10, radius), 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = p.occupied ? '#ffe39a' : '#806f43';
    ctx.beginPath();
    ...
    const device = state.devices.find(d => d.hasScale && Number(d.id) === Number(p.id));
    ctx.fillStyle = '#e8d9ad';
    ctx.font = '11px system-ui';
    ctx.textAlign = 'left';
    ctx.fillText(device ? device.name : `Platform ${p.id}`, x + 13, y + 4);
  }

  for (const d of state.devices.filter(d => d.isJelly)) {
    const [x, y] = xy(d.posX, d.posZ);
    ctx.shadowColor = '#62e4ff';
    ctx.shadowBlur = d.online ? 16 : 0;
    ctx.fillStyle = d.online ? '#62e4ff' : '#42606b';
    ctx.beginPath();
    ctx.arc(x, y, d.isBig ? 9 : 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.fillStyle = '#ccefff';
    ctx.font = '11px system-ui';
    ctx.textAlign = 'left';
    ctx.fillText(d.name, x + 12, y + 4);
  }
}

function roles(d) {
  const list = [];
  if (d.isJelly) list.push(d.isBig ? 'big jelly' : 'jelly');
  if (d.hasScale) list.push('platform');
  if (d.localShowOverride) list.push(`${d.localOverrideMode || 'local'} override`);
  if (!list.length) list.push('controller');
  return list.map(x => `<span class="role">${esc(x)}</span>`).join('');
}

function renderDevices() {
  const grid = document.getElementById('deviceGrid');
  if (!state.devices.length) {
    grid.innerHTML = '<div class="small">Waiting for mDNS discovery...</div>';
    return;
  }
  grid.innerHTML = state.devices.map(d => {
    const sensor = d.sensor || {}, field = d.field || {};
    return `<article class="device-card ${d.online ? '' : 'offline'}" data-key="${esc(d.key)}">
      <div class="device-head">
        <div>
          <div class="device-name">${esc(d.name)}</div>
          <div class="small">ID ${d.id || '—'} · ${esc(d.ip)}</div>
        </div>
        <span class="badge"><i class="dot ${d.online ? 'on' : ''}"></i>${d.online ? 'online' : 'offline'}</span>
      </div>
      <div class="roles">${roles(d)}</div>
      <div class="device-metrics">
        <div class="metric"><b>${esc(d.scene || d.pattern)}</b><span>scene</span></div>
        <div class="metric"><b>${d.hasScale ? pct(sensor.calmness) : pct(field.harmony)}</b><span>${d.hasScale ? 'calm' : 'harmony'}</span></div>
        <div class="metric"><b>${d.hasScale ? pct(sensor.agitation) : (d.rssi ?? '—')}</b><span>${d.hasScale ? 'agitation' : 'RSSI dBm'}</span></div>
      </div>
    </article>`;
  }).join('');
  grid.querySelectorAll('.device-card').forEach(card => card.onclick = () => openDevice(card.dataset.key));
}

function renderEvents() {
  const box = document.getElementById('events');
  if (!state.events.length) {
    box.innerHTML = '<div class="small">No activation events yet.</div>';
    return;
  }
  box.innerHTML = state.events.slice(0, 8).map(e => `<div class="event">
    <b>Platform ${e.platformId || 'test'}</b> at (${num(e.x, 1)}, ${num(e.z, 1)})
    <div class="small">${num(e.weightKg, 1)} kg · gains ${(e.speakerGains || []).map(v => num(v, 2)).join(' / ')}</div>
  </div>`).join('');
}
...
function slider(name, label, value, max = 1, step = .01) {
  return `<label>${label}<input name="${name}" type="range" min="0" max="${max}" step="${step}" value="${esc(value)}"><span class="small" data-value-for="${name}">${num(value)}</span></label>`;
}
...
function hueToHex(hue) {
  ...
  const r = hue2rgb(0, 1, h + 1 / 3), g = hue2rgb(0, 1, h), b = hue2rgb(0, 1, h - 1 / 3);
...
function colorPicker(name, label, hex) {
  return `<label class="color-field"><input name="${name}" type="color" value="${hex}"><span>${label}</span></label>`;
}
...
function renderDialog(d) {
  ...
  document.getElementById('dialogBody').innerHTML = `
    <div class="subpanel">
      ...
      ${d.hasScale ? `<div class="divider"></div><div class="field-grid four">
        <div><b>${num(sensor.weightKg, 1)} kg</b><div class="small">Filtered load</div></div>
        <div><b>${sensor.occupied ? 'Occupied' : 'Empty'}</b><div class="small">Occupancy</div></div>
        <div><b>${esc(sensor.status || 'unknown')}</b><div class="small">HX711</div></div>
        <div><b>${d.clockSynchronized ? 'Synced' : 'Free-running'}</b><div class="small">Show clock</div></div>
      </div>` : ''}
    </div>
    ...
    <div class="subpanel"><h3>Fallback pattern and parameters</h3>
      <form data-submit="pattern">
        <div class="field-grid">
          ...
          <label>Controller override<select name="mode">
            <option value="follow" ${!d.localShowOverride ? 'selected' : ''}>Follow global conductor</option>
            <option value="auto" ${d.localShowOverride && d.localOverrideMode === 'auto' ? 'selected' : ''}>Local auto</option>
            <option value="fallback" ${d.localShowOverride && d.localOverrideMode === 'fallback' ? 'selected' : ''}>Local fallback pattern</option>
            <option value="forest" ${d.localShowOverride && d.localOverrideMode === 'forest' ? 'selected' : ''}>Local ambient forest</option>
            <option value="chorus" ${d.localShowOverride && d.localOverrideMode === 'chorus' ? 'selected' : ''}>Local chorus</option>
            <option value="blackout" ${d.localShowOverride && d.localOverrideMode === 'blackout' ? 'selected' : ''}>Local blackout</option>
          </select></label>
        </div>
    ...
    <div class="subpanel"><h3>Maintenance</h3>
      <form data-submit="ota">
        <div class="field-grid"><label>Firmware .bin<input name="update" type="file" accept=".bin"></label></div>
        <div class="button-row"><button>Upload OTA</button><button type="button" data-action="loadLog">Load log</button></div>
      </form>
...
function wireDialog(d) {
  ...
  document.getElementById('dialogSaveAll').onclick = async () => {
    const rn = new FormData();
    rn.append('name', identityForm.name.value);
    ...
    if (requested && requested !== Number(d.id)) {
      ...
      id.append('deviceId', String(requested));
    ...
    const patternFd = new FormData();
    patternFd.append('pattern', pf.pattern.value);
    ...
    paramsFd.append('hue', hexToHue(pf.hue.value));
    paramsFd.append('hue2', hexToHue(pf.hue2.value));
    ...
    const override = pf.mode.value !== 'follow';
  ...
  body.querySelector('[data-submit=ota]').onsubmit = async e => {
    e.preventDefault();
    await saveDeviceForm(`/api/device/${encodeURIComponent(d.key)}/update`, e.target);
  };
...
document.getElementById('showForm').onsubmit = async e => {
  ...
  fd.set('audioEnabled', e.target.audioEnabled.checked ? 'true' : 'false');
  await postForm('/api/installation/settings', fd);
  state.settingsLoaded = false;
  refresh();
};

document.getElementById('waveForm').onsubmit = async e => {
  e.preventDefault();
  await postForm('/api/installation/test-wave', e.target);
  refresh();
};

document.getElementById('broadcastPatternButton').onclick = async () => {
  const current = prompt('Fallback pattern name:', 'waterfall');
  if (!current) return;
  const fd = new FormData();
  fd.append('pattern', current);
  await postForm('/api/broadcast/pattern', fd);
  refresh();
};

document.getElementById('broadcastUpdateForm').onsubmit = async e => {
  e.preventDefault();
  if (!e.target.update.files.length) { showToast('Choose a firmware .bin first.'); return; }
  await postForm('/api/broadcast/update', e.target);
  e.target.reset();
};
...
setInterval(() => {
  ...
  const fileChosen = [...document.querySelectorAll('input[type=file]')].some(input => input.files && input.files.length);
  if (editing || fileChosen) return;
  refresh();
}, 2000);
